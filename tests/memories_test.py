...
class SparsecoreOffloadTest(jtu.JaxTestCase):
  ...
  @jtu.with_explicit_mesh((2,), 'x')
  def test_sparsecore_fsdp_offload(self, mesh):
    if not (jax.devices()[0].device_kind == "TPU v5"
            or jtu.is_device_tpu_at_least(6)):
      self.skipTest("Does not have a sparsecore present")

    arr1 = jax.device_put(np.arange(32).reshape(8, 4), P('x', None))
    arr2 = jax.device_put(np.arange(24).reshape(4, 6), P('x', None))
  ...
  def test_sparsecore_collective_offload(self):
    if not (jax.devices()[0].device_kind == "TPU v5"
            or jtu.is_device_tpu_at_least(6)):
      self.skipTest("Does not have a sparsecore present")
    ...
    out = f(jnp.arange(32, dtype=np.float32).reshape(8, 4))
